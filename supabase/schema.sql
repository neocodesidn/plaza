...
-- 8. PRIVATE SERVER LINK POOL — admin pre-loads links here, system assigns automatically
create table public.ps_link_pool (
  id uuid primary key default gen_random_uuid(),
  link text not null,
...
-- 9. SITE SETTINGS — simple key/value store for admin-configurable settings
...
-- ============================================================
-- AUTOMATIC PS LINK ASSIGNMENT
-- The moment a trade becomes 'confirmed', grab the oldest available
-- link from the pool, attach it to the trade, and mark it assigned —
-- no admin click required. If the pool is empty, the trade just stays
-- 'confirmed' and shows up in the admin dashboard as "needs a link".
-- ============================================================
create function public.auto_assign_ps_link()
returns trigger as $$
declare
  picked record;
begin
...
-- PS LINK POOL — admin only manages; participants never query this table directly
-- (they see the assigned link via private_servers, which already has its own policy)
